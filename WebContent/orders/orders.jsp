...
<body>
<form action="${pageContext.request.contextPath }/orders/order.do" method="post" name="orderForm">
	...
	<caption style="text-align: center;width:1000px;margin:auto;"><h3>订单表</h3></caption>
	<thead>
		<tr>
			<th>订单编号</th>
			<th>订单日期</th>
	...
	<tbody>
		<order:forEach items="${requestScope.page.li }" var="order" >
			<tr>
				...
				<td>${order.cus.customerName }</td>
				<td>${order.orderRemarks }</td>
				
				<td style="width:140px;">
					<order:if test="${order.orderStatus=='未支付'}">
						<a href="${pageContext.request.contextPath}/orderitem/oi.do?orderCode=${order.orderCode}" class="btn btn-info btn-xs">订单明细</a>
					</order:if>
